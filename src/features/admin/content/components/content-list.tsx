...
import styles from '@/app/admin/content/page.module.css';

interface ContentListProps {
  viewStats: Record<string, number>;
}

export default function ContentList({ viewStats }: ContentListProps) {
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.tableWrap}>
          ...
          <tbody>
            {filtered.map(item => (
              <tr key={item.id} className={styles.row}>
                ...
                <td><span className={styles.views}>{formatViews(viewStats[item.id] || item.popularity || 0)}</span></td>
                <td>
        ...
        {filtered.length === 0 && <div className={styles.empty}><p>Nenhum conteúdo encontrado</p></div>}
      </div>
    </div>
  );
}