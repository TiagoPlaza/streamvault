import { randomUUID } from "crypto";
import fs from "fs";

const API_KEY = "2b269b3d7bfacfb8934eeee70fe138fe";
const BASE_URL = "https://api.themoviedb.org/3";

function escapeString(str) {
  return (str || "")
    .replace(/'/g, "''")
    .replace(/\n/g, " ")
    .replace(/\r/g, " ");
}

// 🎭 Buscar gêneros
async function getGenres() {
  const url = `${BASE_URL}/genre/tv/list?api_key=${API_KEY}&language=pt-BR`;

  const res = await fetch(url);
  const data = await res.json();

  const genresMap = {};

  data.genres.forEach(g => {
    genresMap[g.id] = g.name;
  });

  return genresMap;
}

// 🎬 Buscar videos das série
async function getSeriesVideos(seriesId) {

  const url = `${BASE_URL}/tv/${seriesId}/videos?api_key=${API_KEY}&language=pt-BR`;

  const res = await fetch(url);
  const data = await res.json();

  return data.results || [];

}

// 🎬 Buscar séries populares
async function getSeries(page = 1) {
  const url = `${BASE_URL}/tv/popular?api_key=${API_KEY}&language=pt-BR&page=${page}`;

  const res = await fetch(url);
  const data = await res.json();

  return data.results;
}

// 📺 Detalhes da série
async function getSeriesDetails(seriesId) {
  const url = `${BASE_URL}/tv/${seriesId}?api_key=${API_KEY}&language=pt-BR`;

  const res = await fetch(url);
  return await res.json();
}

// 🎞 Episódios da temporada
async function getSeasonEpisodes(seriesId, seasonNumber) {
  const url = `${BASE_URL}/tv/${seriesId}/season/${seasonNumber}?api_key=${API_KEY}&language=pt-BR`;

  const res = await fetch(url);
  const data = await res.json();

  return data.episodes || [];
}

// 🧾 Série → SQL
function formatSeriesToSQL(series, genresMap, id, videoProvider, videoId) {

  const genres = series.genre_ids
    .map(g => genresMap[g])
    .filter(Boolean);

  return `(
    '${id}',
    'series',
    '${escapeString(series.name)}',
    '${escapeString(series.original_name)}',
    '${escapeString(series.overview)}',
    NULL,
    ${series.first_air_date ? series.first_air_date.split("-")[0] : 2024},
    NULL,
    NULL,
    NULL,
    '${escapeString(JSON.stringify(genres))}',
    '14',
    ${series.vote_average || 7.0},
    ${Math.floor(series.popularity || 0)},
    'published',
    0,
    '${series.backdrop_path ? `https://image.tmdb.org/t/p/w500${series.backdrop_path}` : ""}',
    '${series.poster_path ? `https://image.tmdb.org/t/p/w500${series.poster_path}` : ""}',
    ${videoProvider ? `'${videoProvider.toLowerCase()}'` : "NULL"},
    ${videoId ? `'${videoId}'` : "NULL"},
    '[]',
    NULL,
    'Internacional',
    'Português',
    '[]'
  )`;
}

// 📺 Episódio → SQL
function formatEpisodeToSQL(seriesUUID, episode) {

  const id = randomUUID();

  return `(
    '${id}',
    '${seriesUUID}',
    ${episode.season_number || 1},
    ${episode.episode_number || 1},
    '${escapeString(episode.name || "Episódio")}',
    '${escapeString(episode.overview || "")}',
    ${episode.runtime || "NULL"},
    '${episode.still_path ? `https://image.tmdb.org/t/p/w500${episode.still_path}` : ""}',
    NULL,
    NULL,
    ${episode.air_date ? `'${episode.air_date}'` : "NULL"},
    datetime('now'),
    datetime('now')
  )`;
}

// 🚀 Gerar SQL
export default async function generateSQL() {

  try {

    let seriesSQL = [];
    let episodesSQL = [];

    console.log("🔄 Buscando gêneros...");
    const genresMap = await getGenres();

    console.log("🎬 Buscando séries...");

    for (let i = 1; i <= 5; i++) {

      const seriesList = await getSeries(i);

      for (const series of seriesList) {

        const seriesUUID = randomUUID();

        console.log(`📺 Série: ${series.name}`);

        // detalhes da série
        const details = await getSeriesDetails(series.id);

        // buscar vídeos
        const videos = await getSeriesVideos(series.id);

        const trailer = videos.find(
          v => v.site === "YouTube" && v.type === "Trailer"
        );

        let videoProvider = null;
        let videoId = null;

        if (trailer) {
          videoProvider = trailer.site;
          videoId = trailer.key;
        }

        // SQL da série
        seriesSQL.push(
          formatSeriesToSQL(series, genresMap, seriesUUID, videoProvider, videoId)
        );

        for (const season of details.seasons) {

          if (season.season_number === 0) continue;

          console.log(`   Temporada ${season.season_number}`);

          const episodes = await getSeasonEpisodes(
            series.id,
            season.season_number
          );

          for (const episode of episodes) {

            episodesSQL.push(
              formatEpisodeToSQL(seriesUUID, episode)
            );

          }

        }

      }

    }

    console.log(`✅ Séries: ${seriesSQL.length}`);
    console.log(`✅ Episódios: ${episodesSQL.length}`);

    const sqlContent = `

BEGIN TRANSACTION;

INSERT INTO content (
  id,
  type,
  title,
  original_title,
  description,
  long_description,
  year,
  duration,
  seasons,
  total_episodes,
  genres,
  rating,
  score,
  popularity,
  status,
  featured,
  thumbnail,
  backdrop,
  video_provider,
  video_id,
  cast,
  director,
  country,
  language,
  tags
) VALUES
${seriesSQL.join(",\n")};

COMMIT;
`;
const sqlEpisodes = `

BEGIN TRANSACTION;

INSERT OR IGNORE INTO episodes (
  id,
  series_id,
  season,
  episode,
  title,
  description,
  duration,
  thumbnail,
  video_provider,
  video_id,
  release_date,
  created_at,
  updated_at
) VALUES
${episodesSQL.join(",\n")};

COMMIT;
`;

    fs.writeFileSync("content_seed.sql", sqlContent);
    fs.writeFileSync("episodes_seed.sql", sqlEpisodes);

    console.log("📦 seed.sql gerado com sucesso!");

    return ;

  } catch (error) {

    console.error("❌ Erro:", error);

  }

}